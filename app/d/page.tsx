// LARAPLAY — Page /d (phone/desktop) pour finaliser device flow.
// Si non auth → bouton login Google standard avec callback /d.
// Si auth → form input code + submit /api/auth/device/verify.

import { auth, signIn } from "@/auth";
import { DeviceVerifyClient } from "@/components/DeviceVerifyClient";

export const dynamic = "force-dynamic";

export default async function DevicePage({
  searchParams,
}: {
  searchParams: Promise<{ device?: string }>;
}) {
  const [session, params] = await Promise.all([auth(), searchParams]);
  // Param "device" (pas "code" qui collide avec Google OAuth callback)
  const initialCode = params.device ?? "";

  if (!session?.user?.email) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center px-6 py-8">
        <div className="w-full max-w-sm flex flex-col items-center">
          <h1 className="logo-wordmark text-5xl md:text-6xl mb-8 select-none">LARAPLAY</h1>
          <p className="text-zinc-300 text-center mb-8">
            Connectez-vous pour autoriser votre téléviseur.
          </p>
          <form
            action={async () => {
              "use server";
              const cb = initialCode
                ? `/d?device=${encodeURIComponent(initialCode)}`
                : "/d";
              await signIn("google", { redirectTo: cb });
            }}
            className="w-full"
          >
            <button
              type="submit"
              className="w-full flex items-center justify-center gap-3 bg-white text-zinc-900 font-bold py-3.5 px-4 rounded-lg hover:bg-zinc-100 active:scale-[0.98] shadow-lg transition-all"
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
              </svg>
              Continuer avec Google
            </button>
          </form>
        </div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-black flex items-center justify-center px-6 py-8">
      <div className="w-full max-w-md flex flex-col items-center">
        <h1 className="logo-wordmark text-5xl md:text-6xl mb-6 select-none">LARAPLAY</h1>
        <p className="text-zinc-400 text-center text-sm mb-2">
          Connecté en tant que <span className="text-white font-semibold">{session.user.email}</span>
        </p>
        <p className="text-zinc-300 text-center mb-8">
          Entrez le code affiché sur votre téléviseur :
        </p>
        <DeviceVerifyClient initialCode={initialCode} />
      </div>
    </div>
  );
}
